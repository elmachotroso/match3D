...
namespace QsLib
{
    ...
    /// <summary>
    /// Object pool is a reusable design pattern to create a limited amount of preallocated objects
        ...
        where T : class, new()
    {
        ...
        // which automatically instantiates using default constructor of type T.
        public ObjectPool( int size ) : this()
        {
            for( int i = 0; i < size; ++i )
            {
                PoolItem< T > newItem = new PoolItem< T >();
                newItem.m_object = new T();
    ...
    /// ReadAllToMemory to store all contents of CSV in memory. Lastly, Close if
    /// access to the CSV contents is no longer necessary to save memory.
    /// </summary>
